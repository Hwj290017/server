...
template <typename T> void BaseObject<T>::start(double delay)
{
    ...
    impl_->state_ = BaseImpl<T>::State::kStarted;
}

template <typename T> void BaseObject<T>::stop(double delay)
{
    // 已执行stop
    if (impl_->state_ == BaseImpl<T>::State::kStopped)
    {
        return;
    }
    impl_->ioContext_->queueTask([this, delay]() {
        if (delay <= 0.0)
        {
            if (impl_->stopTask_)
            {
    ...
    impl_->state_ = BaseImpl<T>::State::kStopped;
}
